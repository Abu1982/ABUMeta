...
本文档记录 ABUMeta 开源演示版当前使用到的第三方依赖、第三方服务、外部内容边界与发布注意事项。

## 1. 审计结论

- 当前仓库依赖多个第三方开源项目，但未发现明显的第三方源码整段 vendoring 证据。
- 当前仓库可以公开代码与静态配置，但不应附带运行日志、报告、缓存、抓取结果、数据库与模型缓存。
- 当前基础运行依赖以 `MIT`、`BSD-3-Clause`、`Apache-2.0` 等宽松许可证为主，通常允许商用。
- 当前更需要重点关注的是：外部站点抓取边界、模型权重边界、浏览器组件分发边界、Docker 镜像边界。

## 2. 基础运行依赖

下列依赖已进入基础运行链，可随源码一起公开，但应保留各自许可证与版权声明。

| 项目 | 用途 | 代码证据 | 许可证复核 | 发布判断 |
|---|---|---|---|---|
...
- 当前版本使用 `numpy>=1.26.4`，运行环境元数据显示许可证表达式为 `BSD-3-Clause AND 0BSD AND MIT AND Zlib AND CC0-1.0`。
- 结论：允许公开与商用，但分发时不应只写“NumPy = BSD”，最好保留上游自带 license files 或在 notice 中明确其 bundled components。

### 5.2 `playwright`

- 当前版本使用 `playwright==1.58.0`，代码证据为 `src/execution/lead_capture.py:304`。
- 上游仓库 `microsoft/playwright-python` 的许可证为 `Apache-2.0`。
- 结论：Python 包本身可公开、可商用；如果后续分发浏览器安装产物或打包浏览器二进制，需要额外检查浏览器组件的 notice 与分发规则。

### 5.3 `chromadb`

- 当前版本使用 `chromadb==0.4.22`，代码证据为 `src/memory/retrieval.py:133`。
- 上游仓库 `chroma-core/chroma` 的许可证为 `Apache-2.0`。
- 结论：可作为可选依赖公开与商用；若后续将其作为生产数据库服务分发，建议补充版本锁定、NOTICE 与安全审计。

### 5.4 `docker`

- 当前版本使用 `docker==7.1.0`，代码证据为 `src/execution/sandbox.py:230`。
- 上游仓库 `docker/docker-py` 的许可证为 `Apache-2.0`。
- 结论：Python SDK 本身可公开与商用；但真实部署还涉及 Docker Engine、基础镜像、镜像层内容与宿主机权限，不应仅凭 SDK 许可证直接放行整体部署方案。

### 5.5 `pytest` 与 `pytest-asyncio`

- `pytest` 为 `MIT`，`pytest-asyncio` 为 `Apache-2.0`。
- 结论：二者均可用于公开仓库、内部商业开发与 CI；它们属于开发依赖，不影响主程序是否可商用的判断。
...
说明：第三方 API 是否可商用，不只取决于代码仓许可证，还取决于服务合同、计费条款与输出使用政策。
...
因此，更准确的结论是：当前仓库依赖多个第三方开源项目，但未发现明显的第三方源码直接整段搬运进仓库的证据。
...
- 保留 `LICENSE` 与本文件，作为仓库级公开说明。
- 运行期生成的 `data/action_journal.jsonl`、`data/reports/*.md`、`data/reports/*.json*`、`data/logs/*.log` 应继续被 `.gitignore` 屏蔽。
- 若后续新增第三方依赖，必须同步更新本文件。
- 若后续打包浏览器、模型或镜像，需要额外补充对应的 NOTICE 与商用边界说明。
